...
* 新規に「子ブロック」を追加する際（`Shift + Enter` またはカード内ボタン）、親のインデントに追従して自動的に1段階深い見出しレベル（H4〜H6）で作成されます。
...
* `Shift + Enter` : 選択中のブロックに子ブロックを追加するポップアップを開く
...
* `Shift + O` : 選択中のブロック配下の子ブロックをすべて展開（開く）
* `Shift + C` : 選択中のブロック配下の子ブロックをすべて折りたたむ（閉じる）
